...
const PLAN_LIMITS: Record<string, any> = {
  trial:      { scans: 1,  compare: 1,  bulk: false, lam: false, label: '14-Day Trial' },
...
export default function Dashboard() {
  ...
  // Realtime updates for pending scans
  useEffect(() => {
    if (!user) return
    const channel = sb.channel('pulse_scans_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pulse_scans', filter: `user_id=eq.${user.id}` },
        () => loadScans())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'lam_runs', filter: `user_id=eq.${user.id}` },
        () => loadScans())
      .subscribe()
  ...
  async function triggerScan(url: string, scanType = 'llm', extraUrls: string[] = []) {
    setScanning(true)
    setScanStatus({ msg: `Queuing scan for ${url}...`, type: 'scanning' })
    try {
      // Insert pending scan row
      const { data: scanRow } = await sb.from('pulse_scans').insert({
        user_id: user.id, url, scan_type: scanType, status: 'pending', progress: 0,
        progress_message: 'Queued — starting shortly...'
      }).select().single()

      // Trigger GitHub Actions via klaro-services trigger endpoint
      ...
      if (data.ok) {
        ...
        await sb.from('pulse_scans').update({ status: 'error', error_text: data.error }).eq('id', scanRow?.id)
      }
    } catch (e: any) {
  ...
  async function handleScan() {
    if (mode === 'llm') {
      ...
      await triggerScan(urlInput.trim(), 'llm')
      setUrlInput('')
      ...
      await triggerScan(lamUrl.trim(), 'lam')
      setLamUrl('')
    }
  }
  ...
  // Merge and filter all scans
  const allItems = [
    ...scans.map(s => ({ ...s, _type: 'llm' })),
    ...lamRuns.map(l => ({ ...l, _type: 'lam', overall_score: l.overall_score, report: l.executive_brief }))
  ...
  const avgScore = scans.filter(s => s.overall_score).length
    ? Math.round(scans.filter(s => s.overall_score).reduce((a, b) => a + (b.overall_score || 0), 0) / scans.filter(s => s.overall_score).length)
    : 0
      ...
      <div style={S.topbar}>
        ...
        <div style={S.topRight}>
          {plan === 'trial' && trialDaysLeft !== null && (
            <div style={S.trialBadge}>{trialDaysLeft}d trial left · <a href="/settings" style={{ color: '#818cf8' }}>Upgrade</a></div>
          )}
      ...
      <div style={S.main}>
          ...
          {isLocked(mode) ? (
            <div style={S.upgradeBox}>
              <div style={{ fontWeight: 700, color: 'white', marginBottom: '6px' }}>
                {mode === 'lam' ? '🤖 LAM Audit requires Agency plan ($599/mo)' : mode === 'bulk' ? '⚡ Bulk scanning requires Growth plan ($399/mo)' : '📊 Compare requires Starter plan ($149/mo)'}
              </div>
              <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '12px' }}>
                {mode === 'lam' ? 'Our AI visits your site as a real potential client — full ADA, SOC & conversion audit.' : mode === 'bulk' ? 'Scan multiple sites at once. Great for agencies and monitoring campaigns.' : 'Side-by-side score comparison. Perfect for prospect meetings.'}
              </div>
            ...
            <>
              {mode === 'llm' && (
                <div style={S.inputRow}>
                  <input style={S.urlInput} type="url" placeholder="https://anywebsite.com" value={urlInput} onChange={e => setUrlInput(e.target.value)}
              ...
              {mode === 'compare' && (
                <div>
                  {compareUrls.map((u, i) => (
                    <input key={i} style={{ ...S.urlInput, marginBottom: '8px', width: '100%' }}
                      placeholder={i === 0 ? 'https://your-site.com' : `https://competitor${i}.com`}
                      value={u} onChange={e => { const next = [...compareUrls]; next[i] = e.target.value; setCompareUrls(next) }} />
                  ))}
              ...
              {mode === 'bulk' && (
                <div>
                  <textarea style={{ ...S.urlInput, height: '100px', resize: 'vertical', width: '100%' }}
              ...
              {mode === 'lam' && (
                <div>
                  <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '10px' }}>
                    Our AI visits your site as a real potential client. Tests contact, auth, ADA & SOC. Takes 8-12 min.
                  </div>
                  <div style={S.inputRow}>
                    <input style={S.urlInput} type="url" placeholder="https://yoursite.com" value={lamUrl} onChange={e => setLamUrl(e.target.value)} />
                    <button style={{ ...S.scanBtn, background: 'linear-gradient(135deg,#7c3aed,#a855f7)' }} onClick={handleScan} disabled={scanning}>
        ...
        <div style={S.statsRow}>
          {[
            ...
            { val: scans.filter(s => s.status === 'pending' || s.status === 'scanning').length, label: 'In Progress', color: '#fbbf24' },
          ...
          {filtered.length === 0 && (
            <div style={S.empty}>
              ...
              <div style={{ fontSize: '18px', fontWeight: 700, color: '#334155', marginBottom: '8px' }}>
                {scans.length === 0 && lamRuns.length === 0 ? 'No scans yet' : 'No results for this filter'}
              </div>
              <div style={{ fontSize: '13px' }}>
                {scans.length === 0 && lamRuns.length === 0 ? 'Enter a URL above and click Scan Now to get started' : 'Try a different filter'}
              </div>
            </div>
          )}
          {filtered.map(item => {
              ...
              <div key={item.id} style={S.card}>
                <div style={S.cardHeader}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={S.cardMeta}>
                      ...
                      {isLam && <span style={S.lamTag}> · LAM AUDIT</span>}
                    </div>
                    {isPending && (
                      <div style={S.progressWrap}>
                        ...
                        <div style={S.progressMsg}>{item.progress_message || 'Starting...'}</div>
                ...
                {!isPending && item.status === 'complete' && (
                  <>
                    ...
                    {isLam && (
                      <div style={S.bars}>
                        {[['LAM', item.lam_score], ['ADA', item.ada_score], ['SOC', item.soc_score]].map(([label, val]) => (
                          val ? <div key={label as string} style={S.barItem}>
                            <div style={S.barLabel}><span>{label}</span><span style={{ color: sc(val as number) }}>{val}/100</span></div>
                            <div style={S.barTrack}><div style={{ ...S.barFill, width: `${val}%`, background: sc(val as number) }} /></div>
                          </div> : null
                    ...
                    {/* Summary */}
                    {(r.one_line_verdict || r.plain_english_summary || item.report?.novice_summary) && (
                      <div style={S.summary}>
                        {r.one_line_verdict || r.plain_english_summary || item.report?.novice_summary}
                      </div>
                    )}
                      ...
                      <div style={S.detail}>
                        <div style={S.detailGrid}>
                          ...
                          <div>
                            ...
                            {(item.report.revenue_opportunities || []).map((p: string, i: number) => (
                              <div key={i} style={S.detailItem}>💰 {p}</div>
                            ))}
                          </div>
                        </div>
                    ...
                    {isExpanded && isLam && (
                      <div style={S.detail}>
                        <div style={{ padding: '16px 20px' }}>
                          <div style={{ ...S.detailHead, color: '#818cf8', marginBottom: '10px' }}>📋 Executive Brief</div>
                          <div style={S.detailItem}>{item.executive_brief?.plain_english_summary || '—'}</div>
                          {(item.executive_brief?.top_3_actions || []).map((a: string, i: number) => (
                            <div key={i} style={{ ...S.detailItem, marginTop: '6px' }}><span style={{ color: '#818cf8', fontWeight: 700 }}>0{i + 1}</span> {a}</div>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                {item.status === 'error' && (
                  <div style={{ padding: '12px 20px', color: '#f87171', fontSize: '12px' }}>
                    ❌ {item.error_text || 'Scan failed. Try again.'} &nbsp;
                    <button style={S.ghostBtn} onClick={() => triggerScan(item.url)}>Retry →</button>
                  </div>
                )}

                {item.status === 'cancelled' && (
                  <div style={{ padding: '12px 20px', color: '#475569', fontSize: '12px' }}>
                    Scan cancelled. <button style={S.ghostBtn} onClick={() => triggerScan(item.url)}>Re-scan →</button>
                  </div>
                )}
              </div>
            )
...
const S: Record<string, React.CSSProperties> = {
  ...
  topLogo: { fontSize: '16px', fontWeight: 900, color: 'white', letterSpacing: '-0.3px' },
  accent: { color: '#6366f1' },
  topRight: { display: 'flex', alignItems: 'center', gap: '12px' },
  trialBadge: { fontSize: '11px', background: '#1e2d4a', border: '1px solid #3b4fd8', borderRadius: '20px', padding: '4px 12px', color: '#818cf8' },
  planBadge: { fontSize: '11px', background: '#052e16', border: '1px solid #166534', borderRadius: '20px', padding: '4px 12px', color: '#4ade80' },
  ...
  userEmail: { fontSize: '11px', color: '#64748b', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  ...
  tabLocked: { opacity: 0.6, cursor: 'not-allowed' },
  ...
  lamTag: { color: '#a78bfa', fontWeight: 700 },
  ...
  progressMsg: { fontSize: '11px', color: '#64748b' },
  cancelBtn: { padding: '3px 10px', background: 'transparent', color: '#f87171', border: '1px solid #991b1b', borderRadius: '6px', fontSize: '11px', cursor: 'pointer', fontFamily: 'inherit' },
